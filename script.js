...
let orders = JSON.parse(localStorage.getItem('node_orders') || '[]');
let adminAuthed = false;
const ADMIN_PASSCODE = 'node-admin'; // change this to your own passcode
function saveOrders(){localStorage.setItem('node_orders', JSON.stringify(orders));}

let filters = {category:'all', search:'', maxPrice:250000, minRating:0, sort:'featured'};
...
function renderFilters(){
  ...
  <div class="filters ${filtersOpen?'open':''}" id="filtersPanel">
    ...
    <div class="filter-group">
      <h4>Max price</h4>
      <input type="range" min="10000" max="250000" step="5000" value="${filters.maxPrice}" id="priceRange">
  ...
  <div class="detail">
    <div class="admin-head">
      ...
      <div class="admin-actions">
        <button class="btn btn-ghost" data-action="download-csv">Export all (CSV)</button>
        <button class="btn btn-ghost" data-action="clear-orders" style="color:var(--red);border-color:var(--red);">Clear all orders</button>
      </div>
    </div>
    <div class="admin-stats">
      <div class="admin-stat"><span class="eyebrow">Orders</span><div class="admin-stat-val">${orders.length}</div></div>
      <div class="admin-stat"><span class="eyebrow">Revenue</span><div class="admin-stat-val">${formatPrice(revenue)}</div></div>
    </div>
    ${orders.length===0 ? `
      <div class="empty-state" style="padding:60px 0;">
        <div class="big">No orders yet</div>
        Orders placed through checkout will show up here automatically.
      </div>` : `
      ...
      ${orders.map(o=>`
        <div class="order-card">
          <div class="order-card-head">
            <div>
              <div class="order-num mono">${o.orderNum}</div>
              <div class="order-date">${new Date(o.date).toLocaleString()}</div>
            </div>
            <div class="order-total mono">${formatPrice(o.total)}</div>
          </div>
          <div class="order-customer">${o.customer.fullName || 'No name'} · ${o.customer.email || 'No email'}</div>
          <div class="order-items-mini">${o.items.map(it=>`${it.name} ×${it.qty}`).join(', ')}</div>
          <div class="order-card-actions">
            <button class="btn btn-ghost" data-action="download-pdf" data-ordernum="${o.orderNum}">Download PDF</button>
          </div>
        </div>`).join('')}
    </div>`}
  </div>`;
...
document.addEventListener('click', e=>{
  ...
  switch(action){
    ...
    case 'set-rating':
      ...
      break;
    case 'clear-filters':
      filters = {category:'all', search:'', maxPrice:250000, minRating:0, sort:'featured'};
      render();
      break;
    case 'open-filters':
    ...
    case 'open-admin': {
      if(!adminAuthed){
        ...
        if(code === null) return;
        if(code !== ADMIN_PASSCODE){ showToast('Incorrect passcode'); return; }
        adminAuthed = true;
      }
      navigate('admin');
      break;
    ...
    case 'download-pdf':
      downloadOrderPDF(btn.dataset.ordernum);
      break;
    case 'download-csv':
      downloadOrdersCSV();
      break;
...
function downloadOrderPDF(orderNum){
  ...
  doc.text('NODE — Gear Catalog', 14, y); y += 8;
  doc.setDrawColor(220,220,220); doc.line(14, y, 196, y); y += 10;

  doc.setFontSize(11); doc.setFont('helvetica','normal');
  doc.text(`Order: ${order.orderNum}`, 14, y); y += 6;
  doc.text(`Date: ${new Date(order.date).toLocaleString()}`, 14, y); y += 10;

  doc.setFont('helvetica','bold'); doc.text('Customer', 14, y); y += 6;
  doc.setFont('helvetica','normal'); doc.setFontSize(10);
  [order.customer.fullName, order.customer.email, order.customer.address,
   `${order.customer.city}, ${order.customer.state} ${order.customer.zip}`.trim(),
   order.customer.country].filter(Boolean).forEach(line=>{ doc.text(line, 14, y); y += 5; });
  y += 6;

  doc.setFontSize(11); doc.setFont('helvetica','bold'); doc.text('Items', 14, y); y += 6;
  doc.setFont('helvetica','normal'); doc.setFontSize(10);
  order.items.forEach(it=>{
    doc.text(`${it.name} (${it.sku}) × ${it.qty}`, 14, y);
    doc.text(formatPrice(it.lineTotal), 196, y, {align:'right'});
    y += 6;
  ...
  const totalRows = [['Subtotal', order.subtotal]];
  if(order.discount>0) totalRows.push([`Promo (${order.promo})`, -order.discount]);
  totalRows.push(['Shipping', order.shipping]);
  totalRows.push(['VAT (7.5%)', order.tax]);
  totalRows.forEach(([label,val])=>{
    doc.text(label, 140, y);
    doc.text((val<0?'-':'') + formatPrice(Math.abs(val)), 196, y, {align:'right'});
    y += 6;
  });
  ...
  doc.save(`${order.orderNum}.pdf`);
  showToast('PDF downloaded');
}

function downloadOrdersCSV(){
  if(orders.length===0){ showToast('No orders to export'); return; }
  const header = ['Order #','Date','Customer','Email','Items','Subtotal','Discount','Shipping','VAT','Total','Status'];
  const rows = orders.map(o=>[
    ...
    o.subtotal.toFixed(2), o.discount.toFixed(2), o.shipping.toFixed(2), o.tax.toFixed(2), o.total.toFixed(2),
    o.status
  ]);
  ...
  a.href = url; a.download = 'node-orders.csv';
...
function placeOrder(){
  ...
  const {subtotal,discount,shipping,tax,total,lines} = computeTotals();
  const orderNum = 'NODE-' + Math.floor(100000 + Math.random()*899999);
  const order = {
    orderNum,
    ...
    items: lines.map(l=>({id:l.product.id, name:l.product.name, sku:l.product.sku, qty:l.qty, price:l.product.price, lineTotal:l.product.price*l.qty})),
